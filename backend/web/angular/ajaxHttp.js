                    ...
                    if ($scope.containRequiredProp(user)) {
                        for (var i = 0; i < storage.length; i++) {
                            if ((storage[i].name == user.name) && (storage[i].password == user.password)) {
                                // выполняется проверка на равенство объектов, если условие соблюдается
                                // и они равны, то отображается секретная страница и выполняется выход из цикла
                                $scope.displayPage = "privatePage";
                                break;
                            }
                            if (i == storage.length - 1) {
                                $scope.displayPage = "createAccountPage";
                                // если в хранилище не было найдено пользователя с такими именем и
                                // паролем, то отображается страница создания аккаунта
                            }
                        }
                    }
