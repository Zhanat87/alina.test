ajaxWithoutRest();
function ajaxWithoutRest()
{
    $('html').attr('ng-app', 'MyApp');
    var app = angular.module("MyApp", []);

    app.controller("ajaxWithoutRestCtrl", function($scope) {
            ...
            if ($scope.containRequiredProp(user)) {
                for (var i = 0; i < storage.length; i++) {
                    if (angular.equals(storage[i], user)) {
                        // выполняется проверка на равенство объектов, если условие
                        // соблюдается и они равны то отображается секретная страница
                        // и выполняется выход из цикла
                        $scope.displayPage = "privatePage";
                        break;
                    }
                    if (i == storage.length - 1) {
                        $scope.displayPage = "createAccountPage";
                        // если в хранилище не было найдено пользователя с такими именем
                        // и паролем то отображается страница создания аккаунта
                    }
                }
            }
            ...
            if (angular.isObject(user)) {
                ...
                if ($scope.containRequiredProp(user)) {
                    if (!$scope.checkDuplicates(user)) {
                        ...
                        // метод angular.copy() используется для копирования объекта,
                        // так как если его не использовать, то
